# scripts/math_2022cohort_6points_plot.py
...
INPUT_CSV = BASE_DIR / "math_2022cohort_6points_long.csv"
OUT_PNG   = BASE_DIR / "math_2022cohort_6points_lines.png"
...
def main():
    ...
    # Work out the intended time order from the data:
    # sort by (year, grade, exam_round)
    order_df = (
        df[["year", "grade", "exam_round", "time_label"]]
        .drop_duplicates()
        .sort_values(["year", "grade", "exam_round"])
    )
    ordered_labels = order_df["time_label"].tolist()
    print("\n=== Ordered time labels ===")
    ...
    ax.set_ylim(30, 100)                  # tweak if you like
    ax.set_yticks(range(30, 101, 10))     # 30, 40, ..., 100
    ...
    ax.set_title(
        "2022年度 中1コホートのベネッセ数学スコア推移（中1第1回〜中3第2回）",
        fontproperties=jp_font,
    )
